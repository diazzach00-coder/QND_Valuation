...
.ev-table tr:hover td { background: var(--cream); }
...
.ev-wacc-row .wk { color: #666; }
.ev-wacc-row .wv { font-weight: 500; }
.ev-wacc-total {
    margin-top: 1rem;
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("""
    <div style='font-family:var(--mono);font-size:0.62rem;color:#555;line-height:1.7;'>
    Data: yfinance + FRED<br>
    TV: GGM · EV/EBITDA · EV/EBIT<br>
    Scenarios auto-derived from<br>historical percentiles
    </div>
    """, unsafe_allow_html=True)
...
for label, val, sub in cards:
    ...
    <div class="ev-card">
      ...
      <div class="ev-card-value">{val}</div>
      {"<div class='ev-card-sub'>" + sub + "</div>" if sub else ""}
    </div>"""
card_html += '</div>'
...
for col, (title, best, avoid, examples) in zip([col1,col2,col3], method_guide):
    with col:
        st.markdown(f"""
        <div style="background:var(--cream);padding:1.1rem 1.2rem;border-top:3px solid var(--ink);height:100%;">
          <div style="font-family:var(--mono);font-size:0.65rem;letter-spacing:0.12em;text-transform:uppercase;color:#888;margin-bottom:0.6rem;">{title}</div>
          <div style="margin-bottom:0.7rem;"><span style="font-family:var(--mono);font-size:0.65rem;color:#2e7d52;text-transform:uppercase;">Best for</span><br><span style="font-size:0.82rem;">{best}</span></div>
          <div style="margin-bottom:0.7rem;"><span style="font-family:var(--mono);font-size:0.65rem;color:#c8392b;text-transform:uppercase;">Avoid</span><br><span style="font-size:0.82rem;">{avoid}</span></div>
          <div><span style="font-family:var(--mono);font-size:0.65rem;color:#888;text-transform:uppercase;">Examples</span><br><span style="font-size:0.82rem;">{examples}</span></div>
        </div>
        """, unsafe_allow_html=True)
